...
void ProcessPacket(int ci, unsigned char packet[])
{
	...
	case SC_LOGIN_OK:
	{
		SC_LOGIN_OK_PACKET* mo_packet = reinterpret_cast<SC_LOGIN_OK_PACKET*>(packet);
		g_clients[ci].connected = true;
		active_clients++;
		
		int my_id = ci;
		client_map[mo_packet->id] = my_id;
		g_clients[my_id].id = mo_packet->id;
		g_clients[my_id].x = mo_packet->x;
		g_clients[my_id].y = mo_packet->y;
